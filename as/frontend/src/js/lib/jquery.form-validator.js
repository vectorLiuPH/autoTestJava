...
* @version 2.2.beta.11
* 使用方法
* 1.必须在要使用页面js中调用 $.validate();//表单验证
* 2.在要使用的标签中写
 * data-validation="required"//这个值可参照源码查找具体
 * data-validation-error-msg="请输入产品型号"//提示信息
 * 特殊验证
 * （1）密码验证
 * <input type="password" data-validation="strength" data-validation-strength="1" name="password"/> //验证密码强度
   <input type="password" data-validation-confirm="password"
   data-validation-error-msg="两次密码不一致" data-validation="confirmation"/>//验证确认密码
   注意  data-validation-confirm="password" 必须和密码的name相同
 * （2）长度验证
 * <input id="name" name="name" type="text" data-validation ="length" data-validation-length="5-20"
 * data-validation-error-msg="用户名为5-20个字符" autocomplete="off"  />
 * （3）正则表达式验证
 * <input id="telephone" name="telephone" type="text" data-validation ="custom"
 data-validation-regexp="(^(\d)+(\d|-|\+|\s)*(\d)+$|(^\s*$)$)"
 data-validation-error-msg="请输入正确的联系电话格式"  class="option"/>
 * (4)数字  包含float
 * data-validation ="number"  data-validation-error-msg="请输入价格，价格必须为数字" data-validation-allowing="float"
*
*/
(function($) {
    ...
    var $window = $(window),
        ...
        _setInlineErrorMessage = function($input, mess, conf, $messageContainer) {
            ...
            if( custom ) {
                ...
                if( $mess.length == 0 ) {
                    $mess = $('<small></small>').addClass('help-block').addClass(conf.errorMessageClass);
    ...
    $.fn.setInlineErrorMessage = function(mess, conf){
        _applyErrorStyle($(this), {errorElementClass: 'error'});
        _setInlineErrorMessage($(this), mess, {errorMessageClass: 'error'});
    }
    $.fn.removeErrorStyle = function(mess, conf){
        _removeErrorStyle($(this), {errorElementClass: 'error',errorMessageClass: 'error'});
        _setInlineErrorMessage($(this), mess, {errorMessageClass: 'error'});
    }
    /**
    ...
    $.formUtils = {
        ...
        defaultConfig :  function() {
            return {
                ...
                errorMessageClass : 'error', // class name of div containing error messages when validation fails
        ...
        */
        LANG : {
            ...
            badStrength : '密码强度太弱，请输入大写字母、下划线、数字来增加密码强度',
            badNumberOfSelectedOptionsStart : 'You have to choose at least ',
            badNumberOfSelectedOptionsEnd : ' answers',
            badAlphaNumeric : '只能输入数字、字母、下划线 ',
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction : function(val, $input) {
            ...
            if (!hasTopDomain) {
                ...
                return $.inArray(val, ['i.net', 'q.com', 'q.net', 'x.com', 'x.org', 'z.com', 'z.cn', 'w.org']) > -1;
            } else {
