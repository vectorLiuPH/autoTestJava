 ...
 * Date: 13-11-15
 * Time: 下午3:06
 * Desc: javascript的翻页 调用：GetPagesHtml(当前页数, 记录总数, 每页记录数, 翻页链接, options)
 */

var GetPagesHtml = null;
(function(){
    function getPageLink(curr_page, page, page_url, tag){
        return '<li ' + (curr_page === page ? 'class="active"' : '') +'>'+
            '<a href="'+ page_url.replace('#page#', page) +'">'+ (tag || page) +'</a></li>';
    }
    function getPagesHtml(curr_page, total_num, size, page_url, options){
        ...
        var html = '<ul class="page-ul">';

        if(curr_page > 1){
            html += getPageLink(curr_page, curr_page - 1, page_url, '<span class="icon-prev"></span>');
        }
        ...
        if(curr_page < page_total){
            html += getPageLink(curr_page, curr_page + 1, page_url, '<span class="icon-next"></span>');
        }

        html += '</ul>';
        return html;
    }
    GetPagesHtml = getPagesHtml;
})();